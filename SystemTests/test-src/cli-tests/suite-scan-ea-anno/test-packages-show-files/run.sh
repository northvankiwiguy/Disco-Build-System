...
bmladmin show-files -p | fgrep -v SystemTests
...
bmladmin show-files -p -f %p/zlib | fgrep -v SystemTests

echo
echo "Show the files that are outside the new package"
bmladmin show-files -p -f %np/zlib | fgrep -v SystemTests

echo
echo "Add a single file to zlib/public, and show it"
bmladmin set-file-pkg zlib/public /home/psmith/t/cvs-1.11.23/zlib/libz.a
bmladmin show-files -p -f %p/zlib/public | fgrep -v SystemTests

echo
echo "Show only the private files in zlib"
bmladmin show-files -p -f %p/zlib/private | fgrep -v SystemTests
...
bmladmin show-files -p -f %pkg/zlib | fgrep -v SystemTests
